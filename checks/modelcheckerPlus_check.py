    ...
    if doc.IsWorkshared:
        ...
        for element in elcollector:
            ...
            if worksetKind == "UserWorkset":
                ...
                # determine if element object has name attribute or a valid category
                if hasattr(element.Category, 'Name') and hasattr(element, 'Category') and hasattr(element, 'Name'):
                    if (element.Name not in ('DefaultLocation', '', None) or element.Category.Name not in ('','Other', 0, None)):
                ...
                else: # if element has no name attribute or category add it to the unassigned list, this applies to GraphicsStyle SketchPlanes and other objects.
